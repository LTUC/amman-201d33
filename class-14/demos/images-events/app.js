...
function updateStorage() {
  ...
  let arrayString=JSON.stringify(Goat.allGoats);
   

  localStorage.setItem('goats',arrayString);

  // localStorage.setItem('votes',votesArr);
...
function getItems() {
  ...
  // change it back to a normal array of objects:
  ...
  if (parsedData!==null) {
    ...
    let newGoat=new Goat(parsedData[i].name,parsedData[i].source);

    newGoat.votes=parsedData[i].votes;
    newGoat.shown=parsedData[i].shown;
    // newGoat.drink();
    }
